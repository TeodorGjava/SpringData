...
public class CustomerServiceImpl implements CustomerService {
    ...
    @Override
    public List<ExportOrderedCustomersDTO> findAllOrderByBirthDate() throws IOException {

        List<ExportOrderedCustomersDTO> allOrderByBirthDate = this.customerRepository.findAllOrderByBirthDate().stream()
                .map(customer->MODEL_MAPPER.map(customer,ExportOrderedCustomersDTO.class)).toList();
